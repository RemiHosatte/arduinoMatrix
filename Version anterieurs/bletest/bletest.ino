...
SoftwareSerial ble(12,13);        // For Uno, HM10 TX pin to Arduino Uno pin D2, HM10 RX pin to Arduino Uno pin D3
...
/*  BLECmd(timeout,"AT+NAME?",buffer);
  ...
  F9EB97EE2F88
  */
   BLECmd(timeout,"AT+CO1F9EB97EE2F88",buffer);
...
void loop () {
     ...
     if (ble.available())
     {
     //tempo = ble.readBytes(byteTable,8);
...
/* matrix.setCursor(10, 4);   // start at top left, with one pixel of spacing   
  matrix.setTextColor(matrix.Color333(7,0,0));
  matrix.fillRect(0, 0, 32, 16, matrix.Color888(0, 0, 0));
  matrix.print(numTable); loop() {
